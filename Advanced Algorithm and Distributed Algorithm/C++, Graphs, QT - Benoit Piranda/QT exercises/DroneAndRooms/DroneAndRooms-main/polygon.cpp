...
void Polygon::draw(QPainter &painter) const {
    ...
    if (N > 0) {
        ...
        const double doorWidth = 25.0;

        for (int i = 0; i < N; i++) {
            Vector2D P1 = tabPts[i];
            Vector2D P2 = tabPts[(i + 1) % N];
            ...
            Vector2D u = (1.0 / len) * V;

            // Calculate extremities Q0 and Q1
            Vector2D Q0 = M - (doorWidth / 2.0) * u;
            Vector2D Q1 = M + (doorWidth / 2.0) * u;
